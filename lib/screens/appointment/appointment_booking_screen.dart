                ...
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: Text(
                            DateFormatter.fullDate(appointmentProvider.selectedDate),
      ...
      bottomNavigationBar: Padding(
        ...
        child: ElevatedButton(
          ...
          onPressed: (appointmentProvider.selectedSlot == null ||
                  appointmentProvider.isLoadingSlots)
              ? null
              : () async {
                  // TODO: Cần có logic để lấy `patientId` từ `guardianUser` đã đăng nhập.
                  // Hiện tại, đang tạm sử dụng `userId` thay thế.
                  final success =
                      await appointmentProvider.createAppointment(
                    doctorId: widget.doctorId,
                    patientId: authProvider.user?['_id'],
                  );
                  if (success && mounted) {
